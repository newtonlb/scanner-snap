...
class Gps:
	...
	def getNumbers(self, number, coord='lat'):
		if coord == 'lon':
		...
		while True:
			with serial.Serial('/dev/ttyS4', timeout=100) as ser:
				...
				if re.match(r"^\$GNGGA", line):
					timestamp = data[1]
					deg, minu, sec = self.getNumbers(data[2])
					sec=sec/1000.0
					...
					timestamp = data[5]
					deg, minu, sec = self.getNumbers(data[1])
					sec=sec/1000.0
					lat = self.dms2dd(deg, minu, sec, data[2])

					deg, minu, sec = self.getNumbers(data[3], coord='lon')
					sec=sec/1000.0
					lon = self.dms2dd(deg, minu, sec, data[4])
					# if data[2] == 'N':
					# 	lat = float(data[1])
					# else:
					# 	lat = -float(data[1])
					...
					deg, minu, sec = self.getNumbers(data[3])
					sec=sec/1000.0
					lat = self.dms2dd(deg, minu, sec, data[4])

					deg, minu, sec = self.getNumbers(data[5], coord='lon')
					sec=sec/1000.0
					lon = self.dms2dd(deg, minu, sec, data[6])
					# if data[4] == 'N':
					# 	lat = float(data[3])
					# else:
					# 	lat = -float(data[3])
